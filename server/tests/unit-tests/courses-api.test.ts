...
describe("My test", () => {
  ...
  it("Can perform CRUD on courses", async () => {
    ...
    const getCourse = await $fetch(`/api/courses/${createCourse.id}`)
    ...
    const updateCourse = await $fetch(`/api/courses/${createCourse.id}`, {
      method: "PUT",
      body: {
        title: "2",
        enabled: false,
    ...
    const getCourseAgain = await $fetch(`/api/courses/${updateCourse.id}`)
