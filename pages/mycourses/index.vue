...
<template>
  <v-alert v-model="courseCreatedAlert" type="success" density="compact" title="Course Created" rounded="0" closable
    text="Click on the course you just created in order to add further information."></v-alert>
  <v-alert v-model="courseFailureAlert" type="error" density="compact" title="Course Not Created" rounded="0" closable
    text="Something went wrong. Please try again later."></v-alert>
  <v-container fluid>
    <v-row>
  ...
  <v-dialog v-model="createCourseDialog">
    <v-container fluid>
      <v-row justify="center">
        <CourseCreationForm @course-outcome="handleCourseOutcome" @close="createCourseDialog = false" />
      </v-row>
    </v-container>
  </v-dialog>
</template>
