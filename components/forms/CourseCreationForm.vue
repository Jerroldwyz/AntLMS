...
<template>
  <v-card class="w-50">
    <v-form v-model="valid" @submit.prevent="createCourse">
      ...
      <v-card-text>
        <v-text-field v-model="course.title" label="Title" :rules="titleRules"></v-text-field>
        <v-select v-model="course.tags" label="Tag(s)" :items="tags" :rules="tagRules" multiple chips></v-select>
        <v-file-input v-model="course.thumbnail" label="Thumbnail"></v-file-input>
      </v-card-text>
      ...
      <v-card color="grey-lighten-3">
        <v-container>
          <v-row justify="end">
            ...
            <v-btn class="text-capitalize bg-primary" type="submit" :loading="loading"> Create Course </v-btn>
          </v-row>
        </v-container>
      </v-card>
    </v-form>
